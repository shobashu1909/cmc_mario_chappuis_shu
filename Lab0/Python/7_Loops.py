...
pylog.info('%s', 3*'\t' + 20*'#' + ' FOR AND WHILE LOOPS ' + 20*'#' + 3*'\n')

# range returns a list of integers (Python 2) or a sequence (Python 3)
# returns [0, 1, 2]: includes start value but excludes stop value
pylog.info('Using range method between 0 and 3 %s', list(range(0, 3)))
...
for fruit in fruits:
    ...
    pylog.info("Didn't find the banana yet")
