...
class Animal:
    ...
    def name(self, animal_name):
        ...
        if not isinstance(animal_name, str):
            pylog.warning('You don\'t really know how to name your animals!')
...
pylog.info('I am %s and my name is %s', type(animal), animal.name)
...
class Cat(Animal):
    ...
    Create Cat as a child of Animal class.  You now get access to all the
    attributes and methods of animal class for free!

    """

    def __init__(self, cat_name):
        """You can use the name attribute from the Animal class. """
        super().__init__(name=cat_name)

    def say_meow(self):
        """Make the cat say its name and then meow."""
        pylog.info('My master calls me %s and meow!', self.name)
...
print('Is the cat of type animal (type): %s', isinstance(chat, Animal))
print('Is the cat an animal (isinstance): %s', isinstance(chat, Animal))
